      ...
      {success ? (
        ...
        <div className="flex flex-col items-center gap-5 rounded-xl border border-emerald-200 bg-emerald-50 px-6 py-8 text-center">
          ...
          <div>
            ...
            <p className="mt-2 text-sm leading-relaxed text-emerald-700">
              {t("auth.forgotPassword.successMessage", { identifier: submittedIdentifier })}
            </p>
          </div>
          <Link
            to="/login"
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="flex items-start gap-3 rounded-lg border border-blue-200 bg-blue-50 p-3.5 text-sm text-blue-800">
            ...
            <span>{t("auth.forgotPassword.infoNotice")}</span>
          </div>
          ...
          <div>
            ...
            />
            {!emailValid ? (
              <p className="mt-1 text-xs text-red-600">{t("auth.forgotPassword.emailInvalid")}</p>
            ) : null}
          </div>
            ...
            <div className="flex items-center gap-3">
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: captchaQuery.data?.image || t("auth.forgotPassword.captchaLoading"),
                }}
              />
              <button
                type="button"
          ...
          <button
            ...
            onClick={async () => {
              ...
              if (hasEmail && hasPhone) {
                setFormError(t("auth.forgotPassword.formErrorBoth"));
                return;
              }
              try {
                ...
                setCaptchaSolution("");
                // error shown via forgotPassword.error
              }
            }}
            className="flex w-full items-center justify-center gap-2 rounded-lg py-2.5 text-sm font-semibold text-white shadow-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            style={{ background: "rgb(var(--color-gov-blue))" }}
            onMouseEnter={(e) => {
